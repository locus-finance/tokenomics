...
describe("Midas Claiming", () => {
  it("Deploy contract", async () => {
    const { tokenMock, mockStaking, claim, tree, deployer, treasury } =
  ...
  it("It should claim tokens", async () => {
    const { tokenMock, mockStaking, claim, tree } = await loadFixture(
  ...
  it("It should fail due to bad leaf", async () => {
    const { claim } = await loadFixture(deployContractAndSetVariables);
    let badData = [
  ...
  it("It should emergency exit", async () => {
    const { tokenMock, claim, treasury } = await loadFixture(
      deployContractAndSetVariables
    );
